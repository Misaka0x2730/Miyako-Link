...
typedef struct ADIv5_DP_s {
	...
	void (*seq_out)(struct target_controller *tc, uint32_t MS, int ticks);
	void (*seq_out_parity)(struct target_controller *tc, uint32_t MS, int ticks);
	uint32_t (*seq_in)(struct target_controller *tc, int ticks);
	bool (*seq_in_parity)(struct target_controller *tc, uint32_t *ret, int ticks);
