...
DEF_THREAD_RESOURCE_ARRAY(struct jtag_dev_s, jtag_devs, JTAG_MAX_DEVS+1);
DEF_THREAD_RESOURCE(int, jtag_dev_count);
...
int jtag_scan(const uint8_t *irlens)
{
	int i;
	uint32_t j;
	struct jtag_dev_s *jtag_devs = get_jtag_devs();
	int jtag_dev_count = get_jtag_dev_count();

	target_list_free();

	set_jtag_dev_count(0);
	memset(jtag_devs, 0, sizeof((JTAG_MAX_DEVS+1)*(sizeof(jtag_devs[0]))));
	...
	if (irlens) {
		...
		while((jtag_dev_count <= JTAG_MAX_DEVS) &&
			...
			j += *irlens;
			irlens++;
			set_jtag_dev_count(++jtag_dev_count);
		}
	} else {
		DEBUG("Change state to Shift-IR\n");
		jtagtap_shift_ir();
