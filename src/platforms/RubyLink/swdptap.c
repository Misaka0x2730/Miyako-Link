...
uint32_t swdptap_seq_in(int ticks)
{
    ...
    if (swd_delay_cnt) {
        while (len--) {
            int res;
            res = gpio_get(SWDIO_PIN);
            gpio_put(SWCLK_PIN, 1);
            busy_wait_us_32(swd_delay_cnt);
            ret |= (res) ? index : 0;
            index <<= 1;
            gpio_put(SWCLK_PIN, 0);
            busy_wait_us_32(swd_delay_cnt);
